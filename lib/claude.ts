...
JSON形式のみで回答してください（コードブロックなし）:
{
  ...
  "recommendedOxi": "3%または6%または9%または12%またはAC（目標レベルとダメージに応じて選択：3%=暗め・ハイダメージ、6%=1-2トーンアップ、9%=2-3トーンアップ、12%=ハイリフト専用・健康毛のみ、AC=低アルカリ・敏感肌）",
...
export async function generateColorRecipe(
  ...
  const prompt = `あなたはプロのヘアカラーリストです。以下のデータからプロの技術に基づいた最適なカラーレシピを作成してください。
...
【放置時間の制限】
- 各工程の processingTime は最大20分（目安15〜20分）
- totalTime は実際の壁時計上の所要時間（20〜25分が理想）
- 40分以上の放置はダメージが増大するため絶対に指示しないこと

【根元・毛先の塗り分け（必須）】
根元（新生部）と毛先・中間（既染部／ダメージ部）は必ず別工程・別レシピにすること:
  Step1: 毛先〜中間を先に塗布 → processingTime 10〜15分
         （塗布後5〜7分経過したら Step2 の根元塗布を開始するよう instructions に明記）
  Step2: 根元を後から塗布 → processingTime 15〜20分
         （体温で反応が早いため後塗りでもタイミングが揃う）
この塗り分けにより各部位が適切な時間だけ処理され、過処理によるダメージを防ぐ。

【オキシ（2剤）濃度の選択基準】
目的に応じて以下を使い分け、根元と毛先で濃度を変えてもよい:
  3%  (10vol): 暗くする／同レベル維持／ハイダメージ毛／酸性カラー系
  6%  (20vol): 1〜2トーンアップ／一般的なファッションカラー（標準）
  9%  (30vol): 2〜3トーンアップ／明るいファッションカラー
  12% (40vol): ハイリフトカラー専用（4トーン以上アップ・健康毛のみ）
  AC         : 低アルカリ処方／敏感肌／ジアミンアレルギー代替
agents の role フィールドに濃度を明記すること（例: "オキシ2剤 (6%)"）
  ...
  "steps": [
    {
      ...
      "area": "毛先・中間（既染部）",
      "agents": [
        { "role": "ベース1剤", "name": "商品名", "brand": "ブランド", "code": "コード例:6LA", "amount": 数値, "unit": "g" },
        { "role": "オキシ2剤 (6%)", "name": "オキシ6%", "brand": "ブランド名", "code": "6%", "amount": 数値, "unit": "g" }
      ],
      "processingTime": 10から15の整数,
      "temperature": "room/warm/cool",
      "instructions": "毛先・中間への塗布手順。○分後に根元塗布開始のタイミングも記載すること"
    },
    {
      "stepNumber": 2,
      "area": "根元（新生部）",
      "agents": [
        { "role": "ベース1剤", "name": "商品名", "brand": "ブランド", "code": "コード", "amount": 数値, "unit": "g" },
        { "role": "オキシ2剤 (9%)", "name": "オキシ9%", "brand": "ブランド名", "code": "9%", "amount": 数値, "unit": "g" }
      ],
      "processingTime": 15から20の整数,
      "temperature": "room",
      "instructions": "根元への塗布手順と最終放置時間。毛先と同時に流すタイミングも記載"
    }
  ],
  "totalTime": 実際の壁時計上の所要時間（通常20〜25分・整数）,
  "totalAmount": 総グラム数（整数）,
  "warnings": ["注意事項1", "注意事項2"],
  "allergySafety": "アレルギーに関する安全性コメント",
  "aftercare": "施術後のホームケアアドバイス",
  "notes": "プロとしての施術上の補足コメント（塗り分けタイミング・オキシ選定理由等）"
}`;
