...
import { Camera, Sparkles, Search, Menu, X } from "lucide-react";
      ...
      >
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between">
          ...
          <Link href="/" className="group flex items-center gap-3">
            ...
            <div>
              <span
                className="text-shimmer font-serif text-lg font-semibold tracking-[0.2em]"
            ...
            {NAV_ITEMS.map(({ href, labelEn, icon: Icon }) => {
              ...
              return (
                <Link
                  key={href}
                  href={href}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-medium tracking-[0.15em] transition-all duration-300 ${
                    active
                      ? "bg-gold/10 text-gold border border-gold/20"
                      : "text-pearl-muted hover:text-pearl hover:bg-white/5"
                  }`}
                >
                  <Icon size={13} />
                  {labelEn}
          ...
          {/* Mobile menu button */}
          <button
            className="md:hidden p-2 rounded-lg text-pearl-muted hover:text-gold transition-colors"
            ...
            return (
              <Link
                ...
                className={`flex-1 flex flex-col items-center gap-1 py-3 text-center transition-all ${
                  active ? "text-gold" : "text-pearl-dim"
                }`}
              >
